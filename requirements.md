...
| **P2.1.4** | `free` includes every current 2.0 football feature. Paid gates are **server-side** (`requireEntitlement`). Google, payments and ads stay off until env is set. Ads have **no** HTTP stub; `/api/auth/features` reports `ads: false` and `AdSlot` renders nothing until `ADS_ENABLED` (then it loads AdSense). Plus orgs (`org.ad_free`) omit the hub ad box; Overview and Billing still follow the global flag. |
...
`/api/auth/features` reports `ads: false` unless `ADS_ENABLED`. When ads are on and `ADSENSE_CLIENT` is set, `AdSlot` loads AdSense into `overview.banner` and `hub.sidebar`. Plus leagues and clubs (`org.ad_free`) do not mount `hub.sidebar`; Overview and Billing stay on the global flag. There is no ads HTTP endpoint and no ads `501`. **[Implemented]**
