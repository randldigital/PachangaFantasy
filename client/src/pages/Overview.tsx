...
export default function Overview() {
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      case 'open': return t('league.status.open');
      case 'voting': return t('league.status.voting');
      case 'closed': return t('league.status.closed');
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                ...
                <h1 className="text-2xl font-bold text-white">Pachanga Fantasy</h1>
              </div>
            </div>
          ...
          <div className="flex space-x-3">
            <Dialog open={showJoinLeague} onOpenChange={setShowJoinLeague}>
              ...
              <DialogContent className="bg-slate-800 border-slate-700">
                <DialogHeader>
                  <DialogTitle className="text-white">{t('league.joinLeague')}</DialogTitle>
                </DialogHeader>
              ...
              <Link key={league.id} href={`/league/${league.id}`}>
                <Card className="bg-slate-800/50 border-slate-700 hover:bg-slate-800/70 transition-all duration-200 cursor-pointer group hover:scale-105">
                  <CardHeader className="pb-3">
                    ...
                    {league.description && (
                      <p className="text-slate-400 text-sm line-clamp-2">{league.description}</p>
                    )}
                  </CardHeader>
                  <CardContent className="pt-0">
                    <div className="flex items-center justify-between text-sm">
                      <div className="flex items-center space-x-2 text-slate-400">
                        ...
                        <span>{league.participants?.length || 0} {t('common.participants')}</span>
                      </div>
                      <div className="flex items-center space-x-2 text-slate-400">
