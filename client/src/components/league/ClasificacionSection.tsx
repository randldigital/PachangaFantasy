...
export default function ClasificacionSection({ leagueId, currentUser }: ClasificacionSectionProps) {
    ...
    return (
      <Card className="bg-slate-800/50 border-slate-700">
        <CardContent className="p-8 text-center">
          ...
          <p className="text-slate-400">{t('clasificacion.noDataDescription')}</p>
        </CardContent>
      </Card>
    );
  }
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-4 text-sm text-slate-400">
                      <span>{t('clasificacion.points')}: {player.totalPoints}</span>
                      {player.matchesPlayed && (
                        <span>{t('clasificacion.matches')}: {player.matchesPlayed}</span>
                      )}
                    </div>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              ...
              <div className="text-slate-400 text-sm">
                {t('clasificacion.players')}
              </div>
            </div>
            <div>
              <div className="text-emerald-400 font-bold text-lg">
                {rankings.reduce((total, player) => total + player.totalPoints, 0)}
              </div>
              <div className="text-slate-400 text-sm">
            ...
            <div>
              <div className="text-emerald-400 font-bold text-lg">
                {rankings.length > 0 ? Math.round(rankings.reduce((total, player) => total + player.totalPoints, 0) / rankings.length) : 0}
              </div>
              <div className="text-slate-400 text-sm">
